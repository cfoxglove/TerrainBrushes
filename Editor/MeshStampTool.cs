...
namespace UnityEditor
{
    public class MeshStampTool : TerrainPaintTool<MeshStampTool>
    {
        ...
        public override void OnSceneGUI(SceneView sceneView, Terrain terrain, Texture brushTexture, float brushStrength, int brushSizeInTerrainUnits, float brushRotation = 0.0f, bool holdPosition = false)
        {
            ...
            if (m_LastBrushSize != brushSizeInTerrainUnits)
            {
                m_LastBrushSize = brushSizeInTerrainUnits;
        ...
        public override bool Paint(Terrain terrain, Texture brushTexture, Vector2 uv, float brushStrength, int brushSize, float brushRotation = 0.0f)
        {
            ...
            Rect brushRect = new Rect(uv * new Vector2(terrainSize.x, terrainSize.z) - brushSizeScaled * 0.5f, brushSizeScaled);

            TerrainPaintUtility.PaintContext context = TerrainPaintUtility.BeginPaintHeightmap(terrain, brushRect, "Terrain Paint - Mesh Stamp");
            Material mat = GetPaintMaterial();
            ...
            TerrainPaintUtility.EndPaintHeightmap(context);
