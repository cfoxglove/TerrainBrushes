Shader "CloneStamp" {
    ...
    SubShader {
        ...
        Pass    // 0 clone stamp tool (alphaMap)
        {
            ...
            sampler2D _CloneLocation;
            float4 _WriteRect;      // x, y, width, height

            float4 CloneAlphamap(v2f i) : SV_Target
            {
                // clips drawing to write rect area only
                float2 hv = step(i.texcoord, _WriteRect.zw) * step(_WriteRect.xy, i.texcoord);
                if ((hv.x * hv.y) <= 0.0f)
                    discard;
                ...
                float sampleAlpha = tex2D(_CloneLocation, i.texcoord).r;
        ...
        Pass    // 1 clone stamp tool (heightmap)
        {
            ...
            float4 CloneHeightmap(v2f i) : SV_Target
            {
                ...
                float sampleHeight = UnpackHeightmap(tex2D(_CloneLocation, i.texcoord)) + (HeightOffset / TerrainMaxHeight);
