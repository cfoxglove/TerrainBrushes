    ...
    SubShader {
        ...
        Pass    // 11 Sharpen
        {
            ...
            float4 SharpenHeight(v2f i) : SV_Target
            {
                ...
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord                             ));
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2( xoffset,  0      )));
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2(-xoffset,  0      )));
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2( xoffset,  yoffset))) * 0.75F;
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2(-xoffset,  yoffset))) * 0.75F;
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2( xoffset, -yoffset))) * 0.75F;
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2(-xoffset, -yoffset))) * 0.75F;
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2( 0,        yoffset)));
                avg += UnpackHeightmap(tex2D(_MainTex, i.texcoord + float2( 0,       -yoffset)));
                avg /= 8.0F;
